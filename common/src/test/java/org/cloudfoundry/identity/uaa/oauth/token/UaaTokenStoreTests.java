...
public class UaaTokenStoreTests extends JdbcTestBase {
    ...
    public void test_deserialization_of_uaa_authentication() throws Exception {
        ...
        assertEquals(1, (int)jdbcTemplate.queryForObject("SELECT count(*) FROM oauth_code WHERE code = ?", new Object[] {code}, Integer.class));
        OAuth2Authentication authentication = store.consumeAuthorizationCode(code);
        assertEquals(0, (int)jdbcTemplate.queryForObject("SELECT count(*) FROM oauth_code WHERE code = ?", new Object[] {code}, Integer.class));
    ...
    public void testCleanUpExpiredTokensBasedOnExpiresField() throws Exception {
        ...
        assertEquals(count, (int)jdbcTemplate.queryForObject("SELECT count(*) FROM oauth_code", Integer.class));
        ...
        assertEquals(0, (int)jdbcTemplate.queryForObject("SELECT count(*) FROM oauth_code", Integer.class));
    ...
    public void testCleanUpUnusedOldTokens_MySQL_In_Another_Timezone() throws Exception {
        ...
        try {
            ...
            assertEquals(count, (int)template.queryForObject("SELECT count(*) FROM oauth_code", Integer.class));
            try { store.consumeAuthorizationCode(lastCode); } catch (Exception ignore) {}
            assertEquals(count-1, (int)template.queryForObject("SELECT count(*) FROM oauth_code", Integer.class));
        } finally {
            con.close();
            store = new UaaTokenStore(dataSource);
            legacyCodeServices = new JdbcAuthorizationCodeServices(dataSource);
